...
class Leaked {
 public:
  class Factory {
   public:
    ...
    Leaked* MakeIterator(Commands::Context& context,
      ...
      return new Leaked(allocationFinder, allocationFinder.NumAllocations(),
                        *allocationGraph);
    }
    ...
    void ShowHelpMessage(Commands::Context& context) {
      ...
      output << "Use \"leaked\" to specify"
                " the set of all leaked allocations.\n";
    }
  ...
  Leaked(const Finder<Offset>& allocationFinder, AllocationIndex numAllocations,
  ...
  AllocationIndex Next() {
    while (_index != _numAllocations && !_allocationGraph.IsLeaked(_index)) {
      ++_index;
    }

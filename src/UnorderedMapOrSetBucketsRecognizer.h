...
class UnorderedMapOrSetBucketsRecognizer
 ...
 public:
  ...
  UnorderedMapOrSetBucketsRecognizer(const ProcessImage<Offset>& processImage)
    ...
    const UnorderedMapOrSetAllocationsTagger<Offset>* tagger =
        processImage.GetUnorderedMapOrSetAllocationsTagger();
    if (tagger != 0) {
      _tagIndex = tagger->GetBucketsTagIndex();
    }
  }
  ...
  virtual bool Describe(Commands::Context& context, AllocationIndex index,
    ...
    if (explain) {
       ...
       * The fastest way is to look for an anchor for the buckets or an
       * incoming edge.
       */
    }
    return true;
